...
def align_with_affine_gap(s1, s2):
  ...
  for j in range(max_d, height + 1):
    calc_upper(max_d, j, upper, upper_bt, middle)

    score = BLOSUM[s1[max_d - 1]][s2[j - 1]]
    calc_middle(max_d, j, middle, middle_bt, lower, upper, score)

  for j in range(max_d, width + 1):
    calc_lower(max_d, j, lower, lower_bt, middle)
...
def calc_lower(i, j, lower, lower_bt, middle):
  ...
  if max_v == next_lower:
    lower_bt[i][j] = "down-deletion"
  else:
    lower_bt[i][j] = "diagonal"
...
def calc_upper(i, j, upper, upper_bt, middle):
  ...
  if max_v == next_upper:
    upper_bt[i][j] = "right-insertion"
  else:
    upper_bt[i][j] = "diagonal"
...
def calc_middle(i, j, middle, middle_bt, lower, upper, score):
  ...
  if max_v == next_upper:
    middle_bt[i][j] = "right-insertion"
  elif max_v == next_lower:
    middle_bt[i][j] = "down-deletion"
  else:
    middle_bt[i][j] = "diagonal"
...
def get_new_upper_grid(width, height):
  ...
  for i in range(width):
    ...
    upper_bt[0].append("right-insertion")
...
def get_new_middle_grid(width, height):
  ...
  for i in range(width):
    ...
    middle_bt[0].append("right-insertion")
  ...
  for i in range(height):
    value = OPEN_PENALTY + i * EXTEND_PENALTY
    middle.append([value])
    middle_bt.append(["down-deletion"])
